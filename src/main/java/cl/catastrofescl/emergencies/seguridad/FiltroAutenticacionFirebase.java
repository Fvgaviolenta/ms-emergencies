 ...
 * Valida Bearer, extrae roles de claims y, si faltan, usa headers del gateway o roles por defecto.
 */
@Slf4j
public class FiltroAutenticacionFirebase extends OncePerRequestFilter {
    ...
    private void autenticarDesdeGateway(HttpServletRequest request) {
        ...
        if (rolesDeclarados.isEmpty()) {
            log.warn("Gateway envio uid={} pero sin roles ni fallback configurado", uid);
            return;
        }

        establecerAutenticacion(uid.trim(), rolesDeclarados);
        log.debug("Autenticacion gateway aplicada uid={} roles={}", uid, rolesDeclarados);
    }
